...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const flatCoursewareList = common_vendor.ref([]);
    ...
    const flattenCoursewares = (categories) => {
      if (!Array.isArray(categories))
        return [];
      const result = [];
      const flatten = (items) => {
        items.forEach((item) => {
          if (item.coursewares && Array.isArray(item.coursewares) && item.coursewares.length > 0) {
            result.push(...item.coursewares);
          }
    ...
    const activeFileName = common_vendor.computed(() => {
      if (flatCoursewareList.value.length > 0 && activeIndex.value >= 0) {
        const item = flatCoursewareList.value[activeIndex.value];
        if (item)
    ...
    const finalAmount = common_vendor.computed(() => {
      const p = parseFloat(coursewareData.price) || 0;
    ...
    const selectCoursewareItem = (index) => {
      ...
      const item = flatCoursewareList.value[index];
      if (!item)
        return;
    ...
    const fetchCoursewareData = async () => {
      ...
      try {
        ...
        const response = await api_index.getSpecialCourseWareList(formData);
        if (!response.data) {
          common_vendor.index.showToast({
            title: "未找到课件数据",
            icon: "none"
        ...
        if (data.linked_coursewares) {
          try {
            const parsed = typeof data.linked_coursewares === "string" ? JSON.parse(data.linked_coursewares) : data.linked_coursewares;
            linkedCourseWares.value = parsed;
            flatCoursewareList.value = flattenCoursewares(parsed);
          } catch (e) {
            linkedCourseWares.value = [];
            flatCoursewareList.value = [];
          }
        }
        if (data.lecturer_id) {
          try {
        ...
        if (flatCoursewareList.value.length > 0) {
    ...
    const checkPurchaseStatus = async () => {
      if (!isLogin.value)
        return;
      try {
        const response = await api_index.checkSpecialCourseWarePurchaseStatus({
          UXMID: shopId.value,
          user_id: userId.value,
          product_id: coursewareId.value,
          product_type: "winnowcourseware"
        });
        if (response.data) {
    ...
    const getRecommendations = async () => {
      ...
      if (categoryData.length === 0)
        return;
      try {
        const response = await api_index.getRelatedSpecialCourseWareRecommendations({
          UXMID: shopId.value,
        ...
        if (response.status === "success" && Array.isArray(response.data)) {
          ...
          paymentState.availableCoupons = response.data.filter((userCoupon) => {
            ...
            if (coupon.end_time && parseDate(coupon.end_time) <= now)
              return false;
            if (coupon.usage_limit > 0 && parseFloat(coursewareData.price) < parseFloat(coupon.usage_limit))
              return false;
            return true;
          });
        }
    ...
    const handleDownload = async () => {
      try {
        ...
        const response = await api_index.downloadSpecialCourseware({
          UXMID: shopId.value,
          user_id: userId.value,
    ...
    const goToCourseware = (item) => {
      common_vendor.index.navigateTo({
        url: `/pages/courseware/specialcourseware?id=${item.courseware_id || item.id}`,
        fail: () => common_vendor.index.showToast({
          title: "跳转失败",
          icon: "none"
        })
    ...
    return (_ctx, _cache) => {
      ...
      return common_vendor.e({
        ...
        am: flatCoursewareList.value.length > 0
      }, flatCoursewareList.value.length > 0 ? {
        an: common_vendor.f(flatCoursewareList.value, (item, index, i0) => {
          return common_vendor.e({
            a: "7ffb2e9d-13-" + i0,
        ...
        av: common_vendor.o(closePaymentModal, "63"),
        ...
        aE: common_vendor.o(clearCoupon, "ee")
      } : {}, {
        aF: common_vendor.p({
        ...
        aG: common_vendor.o(toggleCouponList, "5e"),
        ...
        aQ: common_vendor.o(($event) => !isBalanceInsufficient.value && selectPaymentMethod("balance"), "82"),
        aR: wechatIcon,
        aS: paymentState.paymentMethod === "wechat" ? 1 : "",
        aT: paymentState.paymentMethod === "wechat" ? 1 : "",
        aU: common_vendor.o(($event) => selectPaymentMethod("wechat"), "60"),
        ...
        ba: common_vendor.o(confirmPay, "4c"),
        bb: payDisabled.value,
        bc: paymentState.loading.submit,
        bd: common_vendor.o(() => {
        }, "b9"),
        be: common_vendor.o(handlePaymentMaskClick, "95")
      }) : {});
    };
  }
};
const MiniProgramPage = /* @__PURE__ */ common_vendor._export_sfc(_sfc_main, [["__scopeId", "data-v-7ffb2e9d"]]);
wx.createPage(MiniProgramPage);
//# sourceMappingURL=../../../.sourcemap/mp-weixin/pages/courseware/specialcourseware.js.map
